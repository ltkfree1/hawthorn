...
class hawthorn_promo_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'hawthorn_promo_widget', 'description' => __('Display a promo box with an image and link', 'hawthorn') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'hawthorn_promo_widget' );

		/* Create the widget. */
		parent::__construct( 'hawthorn_promo_widget', __('Hawthorn: Promo Box', 'hawthorn'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- Background URL -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_url' ); ?>"><?php esc_html_e( 'Background Image URL', 'hawthorn' ); ?>:</label>
		...
		<!-- Image Text -->
		<p>
			<label for="<?php echo $this->get_field_id( 'heading' ); ?>"><?php esc_html_e( 'Promo Text', 'hawthorn' ); ?>:</label>
		...
		<!-- Link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Promo Link', 'hawthorn' ); ?>:</label>
		...
		<!-- Height -->
		<p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php esc_html_e( 'Promo Box Height', 'hawthorn' ); ?>:</label><br>
		...
		<!-- Margin -->
		<p>
			<label for="<?php echo $this->get_field_id( 'margin' ); ?>"><?php esc_html_e( 'Promo Box Margin Bottom', 'hawthorn' ); ?>:</label><br>
		...
		<!-- New tab -->
		<p>
			<label for="<?php echo $this->get_field_id( 'newtab' ); ?>"><?php esc_html_e( 'Open link in a new tab?', 'hawthorn' ); ?></label>
